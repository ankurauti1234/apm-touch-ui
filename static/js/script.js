...
async function showWiFiPopup() {
    // 1. Close any existing popups FIRST
    closeSettingsPopup();
    closeWiFiPopup();

    // 2. NOW lock the Wi-Fi popup flag so the disconnected warning doesn't show
    wifiPopupIsOpen = true;

    // 3. Hide the disconnected warning if it is currently visible
    closeWifiDisconnectedPopup();
...
function showWifiDisconnectedPopup() {
    ...
    if (wifiDisconnectedPopupShown || wifiPopupIsOpen || settingsPopupIsOpen || disconnectCooldownTimer) {
        return;
    }
...
let settingsPopupIsOpen = false; // 🟢 ADD THIS FLAG
...
function showSettingsPopup() {
    ...
    // 🟢 Lock the flag and instantly close any existing warnings
    settingsPopupIsOpen = true;
    closeWifiDisconnectedPopup();
    closeUsbDisconnectedPopup();
...
function closeSettingsPopup() {
    settingsPopupIsOpen = false; // 🟢 Unlock the flag
...
function getScreensaverContent() {
    ...
    "></div>
        ...
        ${generateAvatarsHtml(activeMembers)}
    </div>
        `;
    }
}
...
function generateAvatarsHtml(activeMembers) {
    const membersToShow = activeMembers.slice(0, 8);
    const count = membersToShow.length;

    // Default sizing for 1, 2, 3, 4, 5 or 6 members (Flexbox row)
    let containerStyle = "display: flex; justify-content: center; gap: 40px;";
    let avatarSize = 130;
    let fontSize = 54;
    let maxTextWidth = 140;

    // Sizing for 5, or 6 members (Grid: 2 rows, 3 columns)
    // if (count >= 5 && count <= 6) {
    //     containerStyle = "display: grid; grid-template-columns: repeat(3, auto); justify-content: center; gap: 30px 40px;";
    //     avatarSize = 100;  // A little smaller
    //     fontSize = 44;
    //     maxTextWidth = 110;
    // }
    // Sizing for 7 or 8 members (Grid: 2 rows, 4 columns)
    if (count >= 7) {
        containerStyle = "display: grid; grid-template-columns: repeat(4, auto); justify-content: center; gap: 30px 40px;";
        avatarSize = 100;  // Smallest to fit the screen
        fontSize = 44;
        maxTextWidth = 110;
    }

    const avatarsList = membersToShow.map((m) => `
        <div style="display: flex; flex-direction: column; align-items: center; gap: 8px;">
            <div style="
                width: ${avatarSize}px;
    ...
    return `
        <div style="${containerStyle} max-width: 90%; margin-bottom: 40px;">
            ${avatarsList}
        </div>
    `;
}
...
async function showScreensaver() {
    ...
    if (!hasActive) {
        ...
        saver.style.background = 'transparent';
    } else {
        ...
        contentHtml = `
            <div style="
                ...
                box-sizing: border-box;
            ">
                <div style="align-self: flex-start; margin-left: 50px; margin-top: 5px; text-align: center;">
                    <div id="clock-time" style="
                        font-size: 90px;
                        font-weight: 700;
                        line-height: 1;
    ...
    // 🟢 ADD THIS: Forcefully resize the clock based on the member count
    const timeEl = document.getElementById('clock-time');
    const dateEl = document.getElementById('clock-date');

    if (timeEl && dateEl && hasActive) {
        if (activeMembers.length <= 6) {
            timeEl.style.fontSize = '130px';
            dateEl.style.fontSize = '45px';
        } else {
            timeEl.style.fontSize = '90px';
            dateEl.style.fontSize = '35px';
        }
    }
    // 🟢 END OF NEW CODE
...
async function updateUsbStatus() {
    if (wifiDisconnectedPopupShown || wifiPopupIsOpen || settingsPopupIsOpen) {
    ...
    try {
        ...
        if (!wifiDisconnectedPopupShown && !wifiPopupIsOpen && !settingsPopupIsOpen && !usbDisconnectedPopupShown) {
            showUsbDisconnectedPopup();
        }
    }
}
...
async function updateAllWarnings() {
    ...
    if (hasWifiIssue) {
        ...
        if (hasUsbIssue) {
            if (!usbDisconnectedPopupShown && !wifiPopupIsOpen && !settingsPopupIsOpen) {
                showUsbDisconnectedPopup();
            }
        } else {
            if (usbDisconnectedPopupShown) {
