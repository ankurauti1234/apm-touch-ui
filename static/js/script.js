// === UI ELEMENTS ===
    ...
    let currentConnectedSSID = null; // Track current Wi-Fi
    ...
    const states = {
        ...
        connect_select: (currentSSID = null) => `
            ...
            ${currentSSID ? `
                ...
                <div class="button-group">
                    <button class="button" onclick="testWiFiConnection()">
            ...
            ` : `
                <div class="button-group">
                    <button class="button" onclick="checkWiFiStatus()">
    ...
    function renderKeys() {
        ...
        keysContainer.innerHTML = layout.map((row, i) => `
    ...
    function insertChar(char) {
        ...
        activeInput.value = text.slice(0, start) + char + text.slice(start);
        activeInput.selectionStart = activeInput.selectionEnd = start + char.length;
        scrollInputIntoView();
        if (shiftActive && /[A-Z]/.test(char)) {
            setTimeout(() => { shiftActive = false; renderKeys(); }, 100);
        }
    }
    ...
    function scrollInputIntoView() {
        ...
        requestAnimationFrame(() => {
            ...
            const keyboard = document.getElementById('virtual-keyboard');
            if (keyboard && inputRect.bottom > keyboard.getBoundingClientRect().top - 100) {
    ...
    async function showWiFiPopup() {
        ...
        popup.innerHTML = `
            <h2><span class="material-icons">wifi</span> Wi-Fi Settings</h2>
            <div id="current-ssid-info" style="display: none; padding: 0.8rem; background: hsl(var(--muted)); border-radius: var(--radius); margin-bottom: 1rem;">
                <div style="display: flex; align-items: center; gap: 0.5rem;">
                    <span class="material-icons" style="color: hsl(var(--success));">wifi</span>
                    <strong>Connected:</strong> <span id="connected-ssid-name"></span>
                </div>
            </div>
            ...
            <div class="button-group">
                <button class="button" id="connect-btn" onclick="connectWiFi()">Connect</button>
        ...
        await loadCurrentWiFiStatusInPopup();
        await scanWiFi();
    }

    async function loadCurrentWiFiStatusInPopup() {
        try {
            const res = await fetch('/api/current_wifi');
            const data = await res.json();
            if (data.success && data.ssid) {
                currentConnectedSSID = data.ssid;
                const info = document.getElementById('current-ssid-info');
                const name = document.getElementById('connected-ssid-name');
                const btn = document.getElementById('connect-btn');
                if (info && name && btn) {
                    name.textContent = data.ssid;
        ...
        try {
            ...
            if (data.success) {
                ...
                data.networks.forEach(n => {
                    ...
                    if (n.ssid === currentConnectedSSID) opt.disabled = true;
    ...
    async function connectWiFi() {
        ...
        try {
            ...
            if (data.success) {
                ...
                setTimeout(() => { closeWiFiPopup(); testWiFiConnection(); }, 2000);
            }
        } catch (e) {
    ...
    async function checkWiFiStatus() {
        try {
            ...
            if (data.success) {
                ...
                if (cdata.success) {
                    currentConnectedSSID = cdata.ssid;
                    render(cdata.ssid);
                } else {
                    showWiFiPopup();
                }
    ...
    async function navigate(state, param = null) {
        ...
        if (state === 'network_test') {
            ...
            render();
            if (param === 'gsm') {
                setTimeout(() => render('success'), 2000);
            }
            return;
        }
