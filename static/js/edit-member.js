/* ==============================================================
   edit-member.js
   Edit Member Name popup + lift handling
   ============================================================== */

   let selectedMemberIndex = -1;  // ← MUST BE DECLARED GLOBALLY
   ...
   function showEditMemberPopup() {
       ...
       popup.innerHTML = `
           <h2 style="margin-top:0;"><span class="material-icons">edit</span> Edit Member Name</h2>
   ...
   async function saveMemberName() {
       const nameInput = document.getElementById('new-code');
       const name = nameInput?.value.trim();
       const err = document.getElementById('edit-error');
   
       if (selectedMemberIndex < 0) {
           showErrorInPopup('Select a member', err);
           return;
       }
       if (!name || name.length > 30) {
           showErrorInPopup('Name must be 1–30 characters', err);
           return;
       }
   
       try {
           const r = await fetch('/api/edit_member_name', {
               method: 'POST',
           ...
           if (d.success) {
               ...
               render();  // This should be your actual render function (check render.js)
